-- Fix Technika write-off act population to use assignment date instead of batch creation date
-- This ensures we track when products were actually USED, not when they were received

-- First, fix the original populate_write_off_act function for Technika
...
BEGIN
  ...
  IF p_module = 'technika' THEN
    ...
    -- Track actual usage from equipment_invoice_item_assignments
  ...
  ELSIF p_module = 'veterinarija' THEN
    -- Veterinarija: Get products used from usage_items (unchanged)
    INSERT INTO public.write_off_act_items (
      act_id,
      product_id,
      product_name,
...
BEGIN
  ...
  ELSIF p_module = 'veterinarija' THEN
    -- Veterinarija: Combine by product + batch_number (unchanged)
    INSERT INTO public.write_off_act_items (
      act_id,
      product_id,
      product_name,
...
COMMENT ON FUNCTION populate_write_off_act IS 'Populates write-off act with detailed batch-level usage. Technika uses assignment date, Veterinarija uses usage_items date.';

-- Enhance write_off_act_items to include stock information
-- Add columns for received quantity and remaining quantity

ALTER TABLE public.write_off_act_items
ADD COLUMN IF NOT EXISTS received_qty NUMERIC,
ADD COLUMN IF NOT EXISTS qty_remaining NUMERIC;

COMMENT ON COLUMN public.write_off_act_items.received_qty IS 'Original quantity received in batch';
COMMENT ON COLUMN public.write_off_act_items.qty_remaining IS 'Quantity remaining in batch after usage';

-- Update the populate function to include stock info

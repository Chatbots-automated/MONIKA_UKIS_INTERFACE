-- Fix populate_write_off_act function to use correct column names
-- Veterinarija products table doesn't have product_code column
...
BEGIN
  ...
  ELSIF p_module = 'veterinarija' THEN
    ...
    -- Note: products table doesn't have product_code column
    INSERT INTO public.write_off_act_items (
      act_id,
      product_id,
      product_name,
    ...
    SELECT
      ...
      p.id,
      p.name,
      NULL, -- products table doesn't have product_code
      p.category::TEXT,

...
BEGIN
  ...
  -- Calculate total weight (package_count Ć— package_weight_g)
  ...
  ELSE
    v_waste_name := v_product_record.name || ' - TuÅÄ¨ios pakuotÄ—s';
  END IF;
...
RAISE EXCEPTION 'Nepakanka atsargÅ³! Turima iÅ viso: %, Reikia: %', v_total_available, NEW.qty;
END IF;
...
RAISE NOTICE 'Sukurtas padalintas ÄÆraÅas: serija %, kiekis %', v_batch.id, v_allocated_qty;
...
BEGIN
  ...
  LOOP
    ...
    SET
      status = 'Cancelled',
      notes = COALESCE(notes || E'\n\n', '') || 'AutomatiÅkai atÅaukta dÄ—l APSÄ–K statuso (' || NOW()::DATE || ')',
      updated_at = NOW()
    ...
    SET
      status = 'AtÅauktas',
      notes = COALESCE(notes || E'\n\n', '') || 'AutomatiÅkai atÅaukta: gyvÅ«nas apsÄ—klintas (APSÄ–K statusas)',
      updated_at = NOW()
...
IF v_total_available < NEW.qty THEN
RAISE EXCEPTION 'Nepakanka atsargÅ³ produktui "%". Turima iÅ viso: %, Reikia: %',
...
RAISE NOTICE 'Vienos serijos nepakanka (%). Bus automatiÅkai padalinta tarp % serijÅ³. Turima iÅ viso: %',
v_qty_left, 
...
BEGIN
  ...
  SELECT
    ...
    COUNT(*) FILTER (WHERE status != 'Baigtas' AND status != 'AtÅauktas')::integer as pending_visits,
    MIN(visit_datetime) FILTER (WHERE status != 'Baigtas' AND status != 'AtÅauktas') as next_visit_date
  FROM animal_visits
...
BEGIN
  ...
  -- CRITICAL FIX: Only trigger when status is TRANSITIONING TO 'APSÄ–K'
  -- Must satisfy ALL conditions:
  -- 1. New status is exactly 'APSÄ–K'
  -- 2. Either it's a new INSERT with 'APSÄ–K', OR
  -- 3. It's an UPDATE where old status was NOT 'APSÄ–K' (actual transition)
  IF v_new_status = 'APSÄ–K' AND
     (TG_OP = 'INSERT' OR (TG_OP = 'UPDATE' AND v_old_status != 'APSÄ–K')) THEN

    RAISE NOTICE 'Auto-cancelling synchronizations for animal % - status transitioning to APSÄ–K (was: %)',
...
COMMENT ON FUNCTION "public"."on_gea_daily_status_change"() IS 'Triggers auto-cancellation of synchronization protocols when animal status transitions TO APSÄ–K.
FIXED: Now only fires on actual status change to APSÄ–K, not on every row update.';
...
BEGIN
  ...
  SET planned_medications = v_updated_medications,
      notes = COALESCE(notes || E'\n\n', '') || '[Sistema atnaujinta: ÄÆveskite faktinÄÆ vaistÅ³ kiekÄÆ prieÅ uÅ¾baigiant vizitÄ…]'
  WHERE id = p_visit_id;
...
begin
  ...
  norm as (
    select
      ...
      nullif(elem->>'liko iki apsiverÅiavimo','')::int                    as liko_iki_apsiversiavimo,
      nullif(elem->>'verÅingumas dienomis','')::int                       as versingumas_dienomis,
      nullif(elem->>'veislinÄ— vertÄ—','')                                  as veisline_verte_txt,

      /* NEW: AB column (string as-is) */
      nullif(coalesce(elem->>'kada versiuosis', elem->>'kada verÅiuosis'),'') as kada_versiuosis
    from src
  )
...
BEGIN
  ...
  IF v_missing_count > 0 THEN
    RETURN QUERY SELECT
      false,
      'PraÅome ÄÆvesti kiekius visiems vaistams (' || v_missing_count || ' trÅ«ksta)',
      v_missing_count;
  ELSE
    RETURN QUERY SELECT true, NULL::text, 0;
  END IF;
...
CREATE TABLE IF NOT EXISTS "public"."animal_visits" (
    ...
    "sync_step_id" "uuid",
    "course_id" "uuid",
    CONSTRAINT "animal_visits_status_check" CHECK (("status" = ANY (ARRAY['Planuojamas'::"text", 'Vykdomas'::"text", 'Baigtas'::"text", 'AtÅauktas'::"text", 'NeÄÆvykÄ™s'::"text"])))
);
...
CREATE OR REPLACE VIEW "public"."vw_animal_visit_analytics" AS
 SELECT "av"."animal_id",
    ...
    "count"(
        CASE
            WHEN ("av"."status" = 'UÅ¾baigtas'::"text") THEN 1
            ELSE NULL::integer
    ...
    "count"(
        CASE
            WHEN ("av"."status" = 'AtÅauktas'::"text") THEN 1
            ELSE NULL::integer
...
CREATE OR REPLACE VIEW "public"."vw_course_schedules" AS
 SELECT "tc"."id" AS "course_id",
    ...
    "count"(DISTINCT "av"."id") FILTER (WHERE (("av"."status" <> 'Baigtas'::"text") AND ("av"."status" <> 'AtÅauktas'::"text"))) AS "pending_visits",
...
CREATE OR REPLACE VIEW "public"."vw_treated_animals" AS
 SELECT "t"."id" AS "treatment_id",
    ...
    'ARTÅŖRAS ABROMAITIS'::"text" AS "veterinarian",
    "t"."notes"
   FROM (("public"."treatments" "t"
     LEFT JOIN "public"."animals" "a" ON (("t"."animal_id" = "a"."id")))
     LEFT JOIN "public"."diseases" "d" ON (("t"."disease_id" = "d"."id")))

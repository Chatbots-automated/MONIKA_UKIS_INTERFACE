...
BEGIN
  ...
  IF NEW.status = 'Baigtas'
    ...
    LOOP
      ...
      -- Deduct from inventory (stock_level table)
      ...
      BEGIN
        ...
        -- Deduct from stock
        UPDATE stock_level
        SET quantity = GREATEST(0, quantity - v_qty_in_primary_units),
            updated_at = now()
        WHERE product_id = (v_medication->>'product_id')::uuid;

        RAISE NOTICE 'Deducted % % (% in primary units) from product %. Stock was: %, now: %',
          v_medication->>'qty',
          v_medication->>'unit',
          v_qty_in_primary_units,
          v_medication->>'product_id',
          v_current_stock,
          v_current_stock - v_qty_in_primary_units;
      END;
    END LOOP;
...
COMMENT ON FUNCTION process_visit_medications IS 'Automatically processes planned medications when visit status changes to Baigtas';

...
WITH treatment_products AS (
  ...
  -- Products from courses (treatment_courses)
  SELECT
    t.animal_id,
    p.id as product_id,
    p.name as product_name,
...
async function applyMigration() {
  console.log('🔧 Applying migration to fix animal cost analytics...');
  ...
  try {
    ...
    console.log('  - vw_animal_cost_analytics (now includes treatment_courses)');
    console.log('  - vw_animal_product_usage (now includes course products)');
  } catch (error) {
    console.error('❌ Migration failed:', error.message || error);
    console.error('\\nℹ️  Please apply the migration manually using the Supabase SQL Editor.');
    console.error('\\n📋 Migration SQL has been generated. You can find it in the script.');
  }
}

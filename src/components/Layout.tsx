...
export function Layout({ children, currentView, onNavigate, onBackToModules }: LayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-gray-50 to-slate-100">
      ...
      <aside className={`fixed left-0 top-0 bottom-0 w-56 bg-gradient-to-b from-emerald-900 via-emerald-800 to-teal-900 shadow-2xl z-30 transition-transform duration-300 lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="h-full flex flex-col">
          <div className="p-3 border-b border-emerald-700/50">
            <div className="flex items-center justify-between mb-2">
              <button onClick={() => setSidebarOpen(false)} className="lg:hidden p-1 hover:bg-emerald-700/50 rounded">
            ...
            <div className="flex items-center gap-2">
              <div className="flex-shrink-0">
                <img
                  ...
                  alt="ZUB Berčiūnai"
                  className="w-10 h-10 rounded-lg bg-white p-0.5 shadow-lg object-contain"
                  onError={(e) => {
              ...
              <div>
                <h1 className="font-bold text-sm text-white leading-tight">ZUB Berčiūnai</h1>
                <p className="text-xs text-emerald-200">VetStock</p>
              </div>
            </div>
          </div>
              ...
              {menuItems.filter(item => hasPermission(item.permission)).map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-2 px-2 py-2 rounded-lg transition-all duration-200 min-h-[40px] touch-manipulation ${
                      isActive
                  ...
                  >
                    <Icon className={`w-4 h-4 flex-shrink-0 ${isActive ? 'text-emerald-700' : ''}`} />
          ...
          <div className="p-2 border-t border-emerald-700/50">
            <button
              ...
              className="w-full flex items-center gap-2 px-2 py-2 text-emerald-50 hover:bg-emerald-700/50 hover:text-white rounded-lg transition-all text-xs min-h-[40px] touch-manipulation active:bg-emerald-600/50"
            >
              <Grid3x3 className="w-4 h-4" />
              <span className="truncate">Moduliai</span>
            </button>
            <div className="text-xs text-emerald-400 text-center pt-1">
              <p>v1.0</p>
            </div>
          </div>
        </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="lg:hidden p-2 hover:bg-slate-100 rounded-lg transition-colors min-w-[40px] min-h-[40px] touch-manipulation active:bg-slate-200"
                >
              ...
              <div className="flex items-center gap-1">
                <button
                  onClick={onBackToModules}
                  className="hidden xl:flex items-center gap-1 px-2 py-1.5 text-xs font-medium text-emerald-700 hover:bg-emerald-50 rounded-lg transition-colors border border-emerald-200 hover:border-emerald-300 min-h-[36px]"
                  title="Modulių pasirinkimas"
                >
                  <Grid3x3 className="w-4 h-4" />
                  <span>Moduliai</span>
                </button>
                <div className="flex items-center gap-2 px-2 py-1.5 bg-gradient-to-r from-emerald-50 to-teal-50 rounded-lg border border-emerald-200 min-h-[36px]">
                  <User className="w-4 h-4 text-emerald-700 flex-shrink-0" />
                  <span className="text-xs font-medium text-emerald-900 truncate max-w-[80px]">
                    {user?.full_name || user?.email?.split('@')[0]}
                  </span>
                </div>
                <button
                  onClick={handleSignOut}
                  className="flex items-center justify-center p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors border border-transparent hover:border-red-200 min-w-[36px] min-h-[36px]"
                  title="Atsijungti"
                >

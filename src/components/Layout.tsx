...
export function Layout({ children, currentView, onNavigate, onBackToModules }: LayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-gray-50 to-slate-100">
      ...
      <aside className={`fixed left-0 top-0 bottom-0 w-56 xl:w-72 bg-gradient-to-b from-slate-900 via-cyan-900 to-slate-900 shadow-2xl z-30 transition-all duration-300 lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="h-full flex flex-col">
          <div className="p-3 xl:p-6 border-b border-cyan-700/50">
            <div className="flex items-center justify-between mb-2 xl:mb-4">
              <button onClick={() => setSidebarOpen(false)} className="lg:hidden p-1 xl:p-2 hover:bg-cyan-700/50 rounded">
                <X className="w-4 xl:w-5 h-4 xl:h-5 text-cyan-200" />
              </button>
            </div>
            <div className="flex items-center gap-2 xl:gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-cyan-200 xl:mt-1">GVET PRO</p>
              </div>
            </div>
          </div>
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-2 xl:gap-3 px-2 xl:px-4 py-2 xl:py-3 rounded-lg transition-all duration-200 min-h-[40px] xl:min-h-[44px] touch-manipulation ${
                      isActive
                        ? 'bg-white text-cyan-900 shadow-lg font-semibold'
                        : 'text-cyan-50 hover:bg-cyan-700/50 hover:text-white active:bg-cyan-600/50'
                    }`}
                  >
                    <Icon className={`w-4 xl:w-5 h-4 xl:h-5 flex-shrink-0 ${isActive ? 'text-cyan-700' : ''}`} />
          ...
          <div className="p-2 xl:p-4 border-t border-cyan-700/50 xl:space-y-3">
            <button
              onClick={onBackToModules}
              className="w-full flex items-center gap-2 xl:gap-3 px-2 xl:px-4 py-2 xl:py-2.5 text-cyan-50 hover:bg-cyan-700/50 hover:text-white rounded-lg transition-all text-xs xl:text-sm min-h-[40px] xl:min-h-[44px] touch-manipulation active:bg-cyan-600/50"
            >
            ...
            <div className="text-xs text-cyan-300 xl:text-cyan-400 text-center pt-1 xl:pt-2">
          ...
          <div className="px-2 xl:px-6 py-2 xl:py-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1 xl:gap-3">
                <button
                  onClick={onBackToModules}
                  className="hidden xl:flex items-center gap-2 px-4 py-2 text-sm font-medium text-cyan-700 hover:bg-cyan-50 rounded-lg transition-colors border border-cyan-200 hover:border-cyan-300"
                ...
                <div className="flex items-center gap-2 px-2 xl:px-4 py-1.5 xl:py-2 bg-gradient-to-r from-cyan-50 to-cyan-100 rounded-lg border border-cyan-200 min-h-[36px]">
                  <User className="w-4 h-4 text-cyan-700 flex-shrink-0" />
                  <div className="flex flex-col">
                    <span className="text-xs xl:text-sm font-medium text-cyan-900 truncate max-w-[80px] xl:max-w-none">
                      {user?.full_name || user?.email}
                    </span>
                    {user && (
                      <span className="text-xs text-cyan-700 hidden xl:block">
          ...
          <div className="px-4 sm:px-6 lg:px-8 py-4">
            <div className="flex items-center justify-between text-sm text-gray-600">
              <div className="flex items-center gap-2">
                ...
                />
                <span>© 2026 Živatkauskų ūkis. Visos teisės saugomos.</span>
              </div>

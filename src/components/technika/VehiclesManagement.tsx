  ...
  const handleOpenVehicleModal = (vehicle?: Vehicle) => {
    if (vehicle) {
      ...
      setVehicleForm({
        ...
        tpm_mokestis: vehicle.tpm_mokestis?.toString() || '',
      });
    } else {
  ...
  const handleSaveVehicle = async () => {
    ...
    try {
      const vehicleData = {
        ...
        tpm_mokestis: vehicleForm.tpm_mokestis ? parseFloat(vehicleForm.tpm_mokestis) : null,
      };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">TPM mokėstis (€)</label>
                <input
                  type="number"
                  step="0.01"
                  value={vehicleForm.tpm_mokestis}
                  onChange={e => setVehicleForm({ ...vehicleForm, tpm_mokestis: e.target.value })}
                  className="w-full border rounded px-3 py-2"
                  placeholder="0.00"
                />
              </div>
